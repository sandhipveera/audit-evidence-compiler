...
# Run-gate token: protects the paid /ws/run + /api/incident endpoints. Reuse an
# existing token if .env already has one, else mint a fresh one.
AEC_RUN_TOKEN_VAL=$(grep -E '^AEC_RUN_TOKEN=' "$REPO_DIR/.env" 2>/dev/null | cut -d= -f2)
[[ -z "$AEC_RUN_TOKEN_VAL" ]] && AEC_RUN_TOKEN_VAL=$(python3 -c "import secrets; print(secrets.token_urlsafe(24))")
...
AEC_RUN_TOKEN=${AEC_RUN_TOKEN_VAL}
...
# Real BOTS v3 dataset (durable: cached on host, restored into the container).
info "Installing BOTS v3 dataset (real 1.9M-event data)…"
bash "$REPO_DIR/scripts/install_botsv3.sh" || warn "BOTS v3 install failed — run manually: bash scripts/install_botsv3.sh"
