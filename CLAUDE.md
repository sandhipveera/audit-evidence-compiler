...
- LangGraph pipeline (`src/aec/agent/graph.py`, `nodes.py`): mapper → spl-gen → validator → mcp → normalizer → **splunk-ml-anomaly** → panel → consensus → formatter → merkle. HITL gates use `interrupt()`, off by default; enable with `--review interactive`.
- **Splunk's own AI runs at runtime** in the `splunk_ml_anomaly` node (`src/aec/splunk/ml_anomaly.py`): it scores the collected evidence with Splunk's in-platform ML — MLTK `fit DensityFunction`/`apply` when the Machine Learning Toolkit is installed, else the built-in `anomalydetection` command (engine behind the Splunk App for Anomaly Detection). Engine is auto-selected (override with `AEC_SPLUNK_ML_ENGINE=auto|mltk|builtin`); the flagged anomalies are merged into the snapshot so all four panel vendors weigh them. Install MLTK with `bash scripts/manage.sh install-mltk <mltk.tgz> <psc.tgz>` (Splunkbase login-gated; falls back to built-in if absent). `SplunkClient` now also supports basic auth (`SPLUNK_PASSWORD`) when `SPLUNK_TOKEN` is unset/rotated.
